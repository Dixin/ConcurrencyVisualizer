...
// https://github.com/dotnet/runtime/blob/main/src/libraries/System.Private.CoreLib/src/System/Diagnostics/Tracing/EventDescriptor.cs
...
public struct EventDescriptor
{
    ...
    public EventDescriptor(int id, byte version, byte channel, byte level, byte opcode, int task, long keywords)
    {
        this.m_id = id switch
        {
            < 0 => throw new ArgumentOutOfRangeException(nameof(id), "Non negative number is required."),
            > ushort.MaxValue => throw new ArgumentOutOfRangeException(nameof(id), $"The ID parameter must be in the range {1} through {ushort.MaxValue}."),
            _ => (ushort)id
        };
        ...
        this.m_task = task switch
        {
            < 0 => throw new ArgumentOutOfRangeException(nameof(task), "Non negative number is required."),
            > ushort.MaxValue => throw new ArgumentOutOfRangeException(nameof(task), $"The ID parameter must be in the range {1} through {ushort.MaxValue}."),
            _ => (ushort)task
        };
    }
}